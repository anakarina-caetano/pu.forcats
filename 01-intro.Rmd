...
bd_enasp_abj <- openxlsx::read.xlsx(xlsxFile = "/home/fcorrea2/Downloads/Banco de dados Cruzamentos.xlsx") %>% 
  filter(instrumento.usado != "Excluir") %>% 
  mutate_all(funs(as.factor))
```

## Introdução

Se você já utilizou R anteriormente, provavelmente já escreveu a expressão `stringsAsFactors = F` quando importava dados usando as funções `read.csv` ou `read.table`. É muito frustrante quando uma coluna de `strings` é lida como um fator, comportam-se de maneira completamente diferente.
...
Em R, fatores são estruturas de dados utilizadas para ordenar `strings`. Formalmente, um fator é definido como um vetor de números inteiros com dois atributos específicos:

- `levels`: Um vetor de textos que indica a relação de ordem entre as variáveis.
- `class`: A string `"factor"`.
...
Fatores são úteis principalmente porque facilitam a ordenação de vetores de texto, principalmente quando estamos fazendo gráficos. Nesta seção vamos ilustrar essa facilidade com um exemplo.

Considere que temos interesse em construir um gráfico que ilustre a diminuição da pressão sanguinea média de um conjunto de indivíduos em função do restaurante em que acabaram de jantar. Para realizar esta tarefa, fomo à praça de alimentação de um shopping e anotamos as pressões arteriais de 10 voluntários conforme eles terminavam suas refeições nos restarantes Max Donalds, Frutaria da vila e Outlet.
...
Mas, sabendo que o Max Donalds oferece uma refeição muito mais calórica do que o Outlet, gostaríamos de atribuir essa ordem ao nosso eixo x. Como poderíamos fazer isso? Usando fatores!
